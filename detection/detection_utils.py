...
def fcos_get_deltas_from_locations(
    ...
    deltas = torch.empty(0, device=device)
    ...
    deltas[(gt_boxes[:, :4] == -1)] = -1 # set background
    deltas = deltas[:, :4]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    output_boxes = torch.empty(0, device=device)
    
    # unnormalize deltas
    deltas = deltas * stride
    
    # Ensure the deltas are non-negative (clipping to zero)
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    # )
    ##########################################################################
    centerness = torch.empty(0, device=device)
    Left = deltas[:, 0]
    Right = deltas[:, 1]
    Top = deltas[:, 2]
    Bottom = deltas[:, 3]
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        x_co, y_co = torch.meshgrid(x, y)

        x_co = x_co.reshape(-1)
        y_co = y_co.reshape(-1)

        location_coords[level_name] = torch.stack((x_co, y_co), dim=1) * level_stride
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
